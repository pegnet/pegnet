...
const (
	...
	PauseStakingIdentity
	ResumeStakingIdentity
)
...
type PegnetStaker struct {
	...
	StakingIdentityState sprStakingIdentityState
...
func NewPegnetStakerFromConfig(c *config.Config, id int, commands <-chan *StakerCommand) *PegnetStaker {
	CheckStakingIdentityAddresses(c)
	p := new(PegnetStaker)
	p.Config = c
	p.ID = id
	p.commands = commands
...
func CheckStakingIdentityAddresses(config *config.Config) {
...
func (p *PegnetStaker) Stake(ctx context.Context) {
	...
	for {
		select {
		case <-ctx.Done():
			return // StakingIdentity cancelled
		case c := <-p.commands:
			p.HandleCommand(c)
		default:
...
func (p *PegnetStaker) HandleCommand(c *StakerCommand) {
	...
	case NewSPRHash:
		p.StakingIdentityState.sprhash = c.Data.([]byte)
	case PauseStakingIdentity:
		// Pause until we get a new start
		p.paused = true
	case ResumeStakingIdentity:
		p.paused = false
	}
}
...
func (p *PegnetStaker) waitForResume(ctx context.Context) {
	...
	for {
		select {
		case <-ctx.Done(): // StakingIdentity cancelled
			return
		case c := <-p.commands:
			p.HandleCommand(c)
			if !p.paused {
