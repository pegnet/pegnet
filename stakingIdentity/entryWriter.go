...
// EntryWriter writes the SPRs to factom once the stakingIdentity is done
type EntryWriter struct {
	// We need an spr template to make the entries
	sprTemplate *spr.StakingIdentityPriceRecord
...
func NewEntryWriter(config *config.Config) *EntryWriter {
	w := new(EntryWriter)
	w.config = config
	w.EntryWritingFunction = w.writeStakingIdentityRecord
	return w
}
