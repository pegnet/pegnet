...
func (*ProtoOPR) Descriptor() ([]byte, []int) {
	return fileDescriptor_0b41b73635d0f3ba, []int{0}
}
...
func (m *ProtoOPR) GetAssets() []float64 {
	if m != nil {
		return m.Assets
	}
	return nil
...
func init() {
	proto.RegisterType((*ProtoOPR)(nil), "opr.ProtoOPR")
}
...
func (m *ProtoOPR) MarshalTo(dAtA []byte) (int, error) {
	var i int
	_ = i
	var l int
	_ = l
	if len(m.Address) > 0 {
		dAtA[i] = 0xa
		i++
		i = encodeVarintProtoOPR(dAtA, i, uint64(len(m.Address)))
		i += copy(dAtA[i:], m.Address)
	}
	if len(m.ID) > 0 {
		dAtA[i] = 0x12
		i++
		i = encodeVarintProtoOPR(dAtA, i, uint64(len(m.ID)))
		i += copy(dAtA[i:], m.ID)
	}
	if m.Height != 0 {
		dAtA[i] = 0x18
	...
	if len(m.Assets) > 0 {
		dAtA[i] = 0x2a
		i++
		i = encodeVarintProtoOPR(dAtA, i, uint64(len(m.Assets)*8))
...
func encodeVarintProtoOPR(dAtA []byte, offset int, v uint64) int {
	for v >= 1<<7 {
...
func (m *ProtoOPR) Size() (n int) {
	...
	if len(m.Assets) > 0 {
		n += 1 + sovProtoOPR(uint64(len(m.Assets)*8)) + len(m.Assets)*8
	}
	if m.XXX_unrecognized != nil {
...
func sovProtoOPR(x uint64) (n int) {
	for {
		n++
		x >>= 7
		if x == 0 {
...
func sozProtoOPR(x uint64) (n int) {
	return sovProtoOPR(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *ProtoOPR) Unmarshal(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		if wireType == 4 {
			return fmt.Errorf("proto: ProtoOPR: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ProtoOPR: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		default:
			iNdEx = preIndex
			skippy, err := skipProtoOPR(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
...
func skipProtoOPR(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		var wire uint64
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowProtoOPR
			}
			if iNdEx >= l {
		...
		case 2:
			...
			if length < 0 {
				return 0, ErrInvalidLengthProtoOPR
			}
			iNdEx += length
			if iNdEx < 0 {
				return 0, ErrInvalidLengthProtoOPR
		...
		case 3:
			for {
				...
				next, err := skipProtoOPR(dAtA[start:])
				if err != nil {
					return 0, err
				}
				iNdEx = start + next
...
var (
	ErrInvalidLengthProtoOPR = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowProtoOPR   = fmt.Errorf("proto: integer overflow")
)
